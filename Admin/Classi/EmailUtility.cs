...
namespace Admin.Classi
{
    public class EmailUtility
    {
        ...
        public static string SendMail_Utente(string nomeUtente, string password, string email)
        {
            try
            {

                var SmtpServer = new AegisImplicitMail.MimeMailer();
                var mail = new AegisImplicitMail.MimeMailMessage();

                mail.From = new System.Net.Mail.MailAddress(ConfigurationManager.AppSettings["User"]);
                mail.To.Add(email);

                // FIX: Cambiato l'oggetto in uno sensato
        ...
        <body>
            <div class='container'>
                ...
                <div class='content'>
                    ...
                    <p>Gentile <strong>{nomeUtente}</strong>,<br>è stato richiesto un reset della password per il tuo account amministratore sul gestionale Bettanini.</p>
                ...
                SmtpServer.AuthenticationMode = AegisImplicitMail.AuthenticationType.Base64;
                SmtpServer.SslType = AegisImplicitMail.SslMode.Ssl;
                ...
                // FIX: Usiamo esplicitamente SendMail
            ...
            {
                // Se c'è un errore ora lo scriverà in console
                System.Diagnostics.Debug.WriteLine("ERRORE INVIO MAIL: " + ex.Message);
