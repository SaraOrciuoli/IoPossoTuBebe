...
using System;
using static Vetrina.Pos.Classes.Barcode;

namespace Vetrina.Pos.Utility
{
...

    {
        ...
        const string bar_code = "alleg riga='{0}',barcode={1}	; QRCODE" + "\n";
        const string courtesy1 = @"CORT R1='[[MSG1]]'	;messaggio di cortesia" + "\n";
        ...
        public string createBody()
        {
            ...
            if (config.client.name != null || !string.IsNullOrEmpty(config.client.name))
            {
                AddFirstCourtesy(config.client.name);
            }
        ...
        private void AddItem(PosItem PosItem)
        {
            ...
            //.Append((item, PosItem.department, PosItem.price, PosItem.name, PosItem.qty));
        }
